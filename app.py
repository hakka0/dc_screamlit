...
def create_navigator_chart(chart_data, target_date, title_prefix=""):
    ...
    upper = base.mark_line(point=True).encode(
        ...
        height=350,
        title=f"{title_prefix} 상세 활동"
    )

    # 4. 하단 네비게이터 차트 (X축이 0~24시로 고정됨)
    lower = base.mark_area().encode(
        x=alt.X('수집시간', axis=alt.Axis(format='%H시', title='구간 선택 (드래그)'),
                scale=alt.Scale(domain=[start_time, end_time])), # [핵심] 0~24시 고정
...
def show_user_detail_modal(nick, user_id, user_type, raw_df, target_date):
    ...
    # [수정] 네비게이터 차트 적용 + Key 추가 (그래프 깨짐 방지)
